...
let value; //Default value of let is undefined
    ...
    let Employee = {first  : "One" , second : "Two"}

    console.log(Employee.first); 

    function Check(NewEmployee) {//Passing a Object as a Parameter
        console.log(NewEmployee.first);
    }
    Check(Employee)

                    //apply and call method
    function Employee1(name,designation)
    {
        this.name=name;
        this.designation =function ()
        {
            console.log(`${this.name} Still Studing`);
        }
    }

           //apply  
    Employee1.apply({},["Kumar","Role"]);
          //call
    Employee1.call({},"Kumar","Role");
    console.log(Employee1.name);
                    //Call Back
    
function addition(n,m,callback)
{
    let ans=n+m;
    callback(ans)
}
